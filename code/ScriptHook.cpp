...
namespace Sentinel
{
	...
	ScriptHook::~ScriptHook()
	{
		if (m_Program)
		{
			...
			{
				auto og_handler = g_Pointers->m_GetNativeHandler(g_Pointers->m_NativeRegistrations, hash);
	...
	void ScriptHook::Ensure()
	{
		...
		if (auto program = g_Pointers->m_ScriptProgramTable->find_script(m_ScriptHash))
		{
			if (program->is_valid())
			{
				HookInstance(program);
